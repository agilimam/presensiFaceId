@php
    $linkBase = 'flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-300';
    $linkActive = 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 font-bold';
    $linkInactive = 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-emerald-600 dark:hover:text-white border border-transparent';
@endphp

<nav class="h-full flex flex-col justify-between bg-white dark:bg-[#1e1e1e] text-gray-900 dark:text-white transition-colors duration-300" aria-label="Menu utama">
    <div>
        ...
        <ul class="p-4 space-y-2 list-none">
            <li class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2 px-4 opacity-50">
                Menu Utama
            </li>
            ...
            <li>
                <a href="{{ route('user.presensi.index') }}"
                   aria-current="{{ request()->routeIs('user.presensi.*') ? 'page' : 'false' }}"
                   class="{{ $linkBase }} {{ request()->routeIs('user.presensi.*') ? $linkActive : $linkInactive }}">
            ...
            <li>
                <a href="{{ route('admin.presensi.index') }}"
                   aria-current="{{ request()->routeIs('admin.presensi.*') ? 'page' : 'false' }}"
                   class="{{ $linkBase }} {{ request()->routeIs('admin.presensi.*') ? $linkActive : $linkInactive }}">
            ...
            <li>
                <a href="{{ route('admin.keluarga.index') }}"
                   aria-current="{{ request()->routeIs('admin.keluarga.*') ? 'page' : 'false' }}"
                   class="{{ $linkBase }} {{ request()->routeIs('admin.keluarga.*') ? $linkActive : $linkInactive }}">
            ...
            <li>
                <a href="{{ route('admin.akun.index') }}"
                   aria-current="{{ request()->routeIs('admin.akun.*') ? 'page' : 'false' }}"
                   class="{{ $linkBase }} {{ request()->routeIs('admin.akun.*') ? $linkActive : $linkInactive }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-4a4 4 0 10-4-4 4 4 0 004 4zm6 0a4 4 0 10-4-4" /></svg>
                    <span>Manajemen Akun</span>
                </a>
            </li>
            <li>
                <a href="{{ route('admin.jadwal.index') }}"
                   aria-current="{{ request()->routeIs('admin.jadwal.*') ? 'page' : 'false' }}"
                   class="{{ $linkBase }} {{ request()->routeIs('admin.jadwal.*') ? $linkActive : $linkInactive }}">
    ...
    <div class="p-6 border-t border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-black/20 transition-colors">
        <button type="button" @click="darkMode = !darkMode"
                ...
                class="w-full flex items-center justify-between px-4 py-2.5 mb-4 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl hover:border-emerald-500 transition-all group">
        ...
        <div class="flex items-center gap-3 mb-6 px-2 text-gray-900 dark:text-white">
            <div class="w-10 h-10 bg-emerald-500/10 rounded-full flex items-center justify-center text-emerald-600 font-bold border border-emerald-100 dark:border-emerald-500/20 shrink-0">
